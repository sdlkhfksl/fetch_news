...
# Logging 配置
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='macd.log', filemode='a')
...
def analyze_market(data, current_price, macd_signal, rsi_signal):
...
def main():
    while True:
        for symbol in SYMBOLS:
            ...
            last_rsi = market_data.iloc[-1]['rsi']
            
            macd_signal = '看涨' if last_macd > last_signal else '看跌'
            rsi_signal = '看涨' if last_rsi < 30 else '看跌' if last_rsi > 70 else '中立'

            # 检查是否有金叉或死叉
            if (prev_macd < prev_signal and last_macd > last_signal) or (prev_macd > prev_signal and last_macd < last_signal):
                logger.info(f"MACD crossover detected for {symbol}")
                ...
                for indicator in indicators:
                    ...
                    time.sleep(20)  # 等待 30 秒后再发起下一个请求
                ...
                send_telegram_message(message)
                logger.info(f"Analysis result for {symbol}: {message}")
