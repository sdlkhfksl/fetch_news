...
# Binance API setup
API_KEY = 'YourBinanceAPIKey'  # Your Binance API Key
API_SECRET = 'YourBinanceAPISecret'  # Your Binance API Secret
...
def check_prices_and_notify():
    global last_checked_btc_level, last_checked_eth_level
    messages = []
    ...
    if len(price_data['BTCUSDT']) == 2:
        bitcoin_current_price = price_data['BTCUSDT'][-1]
        bitcoin_prev_price = price_data['BTCUSDT'][0]
    ...
    if len(price_data['ETHUSDT']) == 2:
        ethereum_current_price = price_data['ETHUSDT'][-1]
        ethereum_prev_price = price_data['ETHUSDT'][0]
...
# Main function to periodically check prices and notify
def main():
    while True:
        try:
