...
// 交易量显示部分
buy_volume = volume * (close > open ? 1 : 0)
sell_volume = volume * (close < open ? 1 : 0)

plot(buy_volume, title="Buy Volume", color=color.green, style=plot.style_histogram, linewidth=2)
plot(sell_volume, title="Sell Volume", color=color.red, style=plot.style_histogram, linewidth=2)
...
// Plotting
plot(macd, color=col_macd, title="MACD")
plot(signal, color=col_signal, title="Signal")
plot(hist, title="Histogram", color=(hist >= 0 ? (hist[1] < hist ? col_grow_above : col_fall_above) : (hist[1] < hist ? col_grow_below : col_fall_below)), style=plot.style_histogram)
