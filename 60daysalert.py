...
def get_price_from_coinmarketcap():
    cmc_ids = ','.join(COINMARKETCAP_IDS.values())
    ...
    try:
        ...
        price_data = response.json()
        prices = {}
        for currency, cmc_id in COINMARKETCAP_IDS.items():
            prices[currency] = price_data['data'][str(cmc_id)]['quote']['USD']['price']
        return prices
    except requests.RequestException as e:
        logging.error("Error fetching data from CoinMarketCap")
        logging.exception(e)
        return None
...
def update_and_check_price(currency_id, price):
    ...
    if not prices[currency_id] or prices[currency_id][-1][0] != today:
        ...
        prices[currency_id].append((today, price, price))
        logging.info(f"Added new price for {currency_id}: {price}")
    else:
        _, low, high = prices[currency_id].pop()
        low = min(low, price)
        high = max(high, price)
        prices[currency_id].append((today, low, high))
        logging.info(f"Updated price for {currency_id}: low=${low}, high=${high}")

    lowest_alert, highest_alert = None, None

    # Check lowest and highest prices for each time range
    for days in reversed(time_ranges):
        relevant_prices = list(prices[currency_id])[-days:]

        # Find the lowest price in the relevant range
        lowest_today = min(relevant_prices, key=lambda x: x[1], default=(today, float('inf'), float('-inf')))
        if price < lowest_today[1]:
            lowest_alert = days

        # Find the highest price in the relevant range
        highest_today = max(relevant_prices, key=lambda x: x[2], default=(today, float('inf'), float('-inf')))
        if price > highest_today[2]:
            highest_alert = days

    # Send messages for the longest time range only
    messages = []
    if lowest_alert:
        messages.append(f"{currency_id.upper()} new low price alert in {lowest_alert}-day range! Current price: ${price}")

    if highest_alert:
        messages.append(f"{currency_id.upper()} new high price alert in {highest_alert}-day range! Current price: ${price}")
...
def main():
    ...
    while True:
        ...
        try:
            ...
            logging.error("An unexpected error occurred in the main loop")
            logging.exception(e)
