...
def update_and_check_price(currency_id, price):
    ...
    for days in time_ranges:
        ...
        if price < lowest_today[1]:
            messages.append(f"{currency_id.upper()} new low price alert in {days}-day range! Current price: ${price}")

        # 检查最高价格
        highest_today = max(relevant_prices, key=lambda x: x[1])
        if price > highest_today[1]:
            messages.append(f"{currency_id.upper()} new high price alert in {days}-day range! Current price: ${price}")

    if messages:
        message = "\n".join(messages)
        logging.info(f"Sending combined message for {currency_id}: {message}")
