...
def load_prices():
    def date_parser(date_str):
        return datetime.fromisoformat(date_str).date()  # 将字符串转换为日期对象

    try:
        with open('prices.json', 'r') as f:
            data = json.load(f)
            for currency in CURRENCY_IDS:
                if currency in data['lowest_prices']:
                    lowest_prices[currency] = deque(
                        [(date_parser(date), price) for date, price in data['lowest_prices'][currency]],
                        maxlen=MAX_DAYS
                    )
                if currency in data['highest_prices']:
                    highest_prices[currency] = deque(
                        [(date_parser(date), price) for date, price in data['highest_prices'][currency]],
                        maxlen=MAX_DAYS
                    )
    except FileNotFoundError:
        logging.info("No previous price data found, starting fresh.")
