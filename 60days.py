...
def save_prices():
    def date_converter(o):
        if isinstance(o, datetime.date):
            return o.isoformat()  # 将日期对象转换为 ISO 格式的字符串
        raise TypeError("Type not serializable")
    
    data = {
        'lowest_prices': {currency: [(date.isoformat(), price) for date, price in lowest_prices[currency]] for currency in CURRENCY_IDS},
        'highest_prices': {currency: [(date.isoformat(), price) for date, price in highest_prices[currency]] for currency in CURRENCY_IDS}
    }
    with open('prices.json', 'w') as f:
        json.dump(data, f, indent=4, default=date_converter)
    logging.info("Price data saved to local file.")
...
def load_prices():
    try:
        with open('prices.json', 'r') as f:
            data = json.load(f)
            global lowest_prices, highest_prices
            lowest_prices = {currency: deque([(datetime.fromisoformat(date), price) for date, price in values], maxlen=MAX_DAYS)
                             for currency, values in data.get('lowest_prices', {}).items()}
            highest_prices = {currency: deque([(datetime.fromisoformat(date), price) for date, price in values], maxlen=MAX_DAYS)
                             for currency, values in data.get('highest_prices', {}).items()}
        logging.info("Price data loaded from local file.")
    except FileNotFoundError:
        logging.info("No local price data file found. Starting with empty data.")
    except Exception as e:
        logging.error(f"Error loading price data from file: {e}")
